...
use crate::api::{DeleteProject, UpdateProjectNameToken};
use crate::common::{Project, ProjectType, Token};
...
pub fn ProjectSettings() -> impl IntoView {
    ...
    let update_project_action = ServerAction::<UpdateProjectNameToken>::new();
    let toast_context = expect_context::<ToasterContext>();
    let tc = toast_context.clone();

    Effect::new(move |_| {
        if update_project_action.version().get() > 0 {
            tc.toast("Update Success", ToastVariant::Success);
            set_trigger.set(());
        }
    });

    let confirm_delete_dialog = NodeRef::<leptos::html::Dialog>::new();
    ...
    Effect::new(move |_| {
        if delete_project_action.version().get() > 0 {
            toast_context.toast("Project Deleted", ToastVariant::Success);
    ...
    view! {
        <Transition>
                    ...
                    Some(project_type) => {
                        match project_type {
                            ...
                            ProjectType::Container { tokens, .. } => {
                                let (tokens, set_tokens) = signal(tokens);
                                view! {
            ...
            <dialog
                node_ref=confirm_delete_dialog
            ...
            >
                ...
                <div class="flex gap-2">
                    <button
                        ...
                        on:click=move |_| {
                            delete_project_action.dispatch(DeleteProject { id: id.get() });
                        }
                    >
